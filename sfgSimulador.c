...
int sfg_simular(double tiempo, Cuerpo *cuerpos)
{
    ...
    int i, j;
    double distancia, fuerzaVector;

    double distanciaAlCuadrado;

    float masaConstanteCiclo;

    float vectorX, vectorY, tiempoCuadrado;

    float aceleracionEnX, aceleracionEnY;
    ...
    for (i = 0; i < numCuerposSimulados; ++i)
    {
        ...
        masaConstanteCiclo = 6.672E-11 * masas[i];
        ...
        for (j = i + 1; j < numCuerposSimulados; ++j)
        {
            ...
            distanciaAlCuadrado = (vectorX * vectorX) + (vectorY * vectorY);

            distancia = sqrt(distanciaAlCuadrado);
            ...
            fuerzaVector = (masaConstanteCiclo * masas[j]) / (distancia * distanciaAlCuadrado);

            // Dado que fuerzaVector es la fuerza lineal, hay que transformarlo a forma vectorial
            fuerzas[0][i][j] = (float)(fuerzaVector * vectorX); // Fuerza en coordenada X
            fuerzas[0][j][i] = fuerzas[0][i][j];

            fuerzas[1][i][j] = (float)(fuerzaVector * vectorY); // Fuerza en coordenada Y
    ...
    for (i = 0; i < numCuerposSimulados; ++i)
    {
        ...
        aceleracionEnX = vectorX / masas[i];
        aceleracionEnY = vectorY / masas[i];
        ...
        posiciones[0][i] = posiciones[0][i] + velocidades[0][i] * tiempo + 0.5 * aceleracionEnX * tiempoCuadrado;
        posiciones[1][i] = posiciones[1][i] + velocidades[1][i] * tiempo + 0.5 * aceleracionEnY * tiempoCuadrado;

        cuerpos[i].posicionX = posiciones[0][i];
        cuerpos[i].posicionY = posiciones[1][i];

        // La formula para la velocidad es v = v0 + a*t
        velocidades[0][i] = velocidades[0][i] + aceleracionEnX * tiempo;
        velocidades[1][i] = velocidades[1][i] + aceleracionEnY * tiempo;
